package com.myfauzan.myalarmmanager
...
class AlarmReceiver : BroadcastReceiver() {
    ...
    fun setOnTimeAlarm(context: Context, type: String, date: String, time: String, message: String) {
        ...
        calendar.set(Calendar.YEAR, parseInt(dateArray[0]))
        calendar.set(Calendar.MONTH, parseInt(dateArray[1]) - 1)
        calendar.set(Calendar.DAY_OF_MONTH, parseInt(dateArray[2]))
        calendar.set(Calendar.HOUR_OF_DAY, parseInt(timeArray[0]))
        calendar.set(Calendar.MINUTE, parseInt(timeArray[1]))
    ...
    fun cancelAlarm(context: Context, type: String) {
        val alarmManager = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        val intent = Intent(context, AlarmReceiver::class.java)
        val requesCode = if (type.equals(TYPE_ONE_TIME, ignoreCase = true)) ID_ONETIME else ID_REPEATING
        val pendingIntent = PendingIntent.getBroadcast(context, requesCode, intent, PendingIntent.FLAG_IMMUTABLE)
